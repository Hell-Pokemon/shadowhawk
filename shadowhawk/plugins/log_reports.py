...
@Client.on_message(~filters.chat(config['logging']['regular']) & filters.regex(r'(?:^|\s+)@admins?(?:$|\W+)|^[/!](?:report|admins?)(?:$|\W+)') & filters.group)
@log_errors
async def log_reports(client, message):
	...
	async with lock:
		...
		text = f'<b>Report Event</b> [#REPORT]\n- <b>Chat:</b> {chat_text} '
		...
		if not getattr(reply, 'empty', True):
			...
			if reply.from_user:
				...
				user_text = name_escape(reply.sender_chat.title)
				link = await get_chat_link(client, reply)
				user_text = f'<a href="{link}">{user_text}</a>'
			...
			reported.append((reply.chat.id, reply.message_id))
		await log_chat(text)
